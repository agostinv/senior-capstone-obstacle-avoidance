...
void Camera_Setup(uint8_t* CAM_RESET)
{
	HAL_Delay(2500);	//2500 ms delay
	uint8_t rec[5];
	while(!send_USART(CAM_RESET, rec)) {}	//repeats until successful message sent and message received

	//if
}

// abstracted function for general USART exchange, note that size of rec must be known beforehand for this to work
bool send_USART(uint8_t* comm, uint8_t* rec)
{
	int i = 0;
	while(HAL_USART_Transmit(&husart1, comm, sizeof(comm), HAL_MAX_DELAY) != HAL_OK)
	{
		if (i > 100)
			return false;
		i++;
	...
	i = 0;
	while(HAL_USART_Receive(&husart1, rec, sizeof(rec), HAL_MAX_DELAY) != HAL_OK)
	{
		if (i > 100)
			return false;
		i++;
